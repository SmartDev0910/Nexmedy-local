...
export default function UiUx() {
  ...
  //   console.log("pathname", router.pathname.slice(1).split("/")[1]);
...
function SectionEight({ data }) {
  ...
  } else if (data.path === 'frontend-development') {
    clsName = {
  ...
  return (
    <section
      className={`c-8 ${
        data.path === 'frontend-development' || data.path === 'graphic-design'
          ? 'yellowBg'
          : ''
      }`}
    >
...
function SectionEleven({ path }) {
  ...
  const cardData = HomeCourseCards.filter((i) => {
